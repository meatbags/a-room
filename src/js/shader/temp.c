...
uniform uint nodeUniform1;
uniform uint nodeUniform4;
uniform vec2 nodeUniform9;
uniform uint f_nodeUniform19;
uniform uint f_nodeUniform23;
uniform uint f_nodeUniform24;
uniform uint f_nodeUniform25;
uniform uint f_nodeUniform26;
uniform uint f_nodeUniform27;
...
void main() {
	...
	nodeVar5 = texture2D( nodeUniform0, nodeVar2 ).x;
	depth = nodeVar5;
	...
	viewPosition = ( ( cameraProjectionMatrixInverse * vec4( ( ( vec3( nodeVarying3.x, ( 1.0 - nodeVarying3.y ), depth ) * vec3( 2.0 ) ) - vec3( 1.0 ) ), 1.0 ) ).xyz / vec3( ( cameraProjectionMatrixInverse * vec4( ( ( vec3( nodeVarying3.x, ( 1.0 - nodeVarying3.y ), depth ) * vec3( 2.0 ) ) - vec3( 1.0 ) ), 1.0 ) ).w ) );
	...
	nodeVar10 = texture2D( nodeUniform3, nodeVar7 );
	...
	for ( uint i = 0u; i < ROTATION_COUNT; i ++ ) {
		...
		for ( uint i = 0u; i < STEP_COUNT; i ++ ) {
			...
			if ( ( true == true ) ) {
				kernelRotation = vec2( 1.0, -1.0 );
			} else {
				kernelRotation = vec2( -1.0, 1.0 );
			}

			sampleUV = (nodeVarying3 + (uvOffset * kernelRotation));
			...
			nodeVar19 = texture2D( nodeUniform0, nodeVar16 ).x;
			sampleViewPosition = ( ( cameraProjectionMatrixInverse * vec4( ( ( vec3( sampleUV.x, ( 1.0 - sampleUV.y ), nodeVar19 ) * vec3( 2.0 ) ) - vec3( 1.0 ) ), 1.0 ) ).xyz / vec3( ( cameraProjectionMatrixInverse * vec4( ( ( vec3( sampleUV.x, ( 1.0 - sampleUV.y ), nodeVar19 ) * vec3( 2.0 ) ) - vec3( 1.0 ) ), 1.0 ) ).w ) );
			...
			if ( ( float( nodeVar29 ) > 0.0 ) ) {
				...
				if ( nodeVar32 ) {
					...
					nodeVar33 = nodeVar30;
					lightColorUV = vec2( nodeVar33.x, 1.0 - nodeVar33.y );

				} else {

					lightColorUV = nodeVar30;

				}

				nodeVar34 = texture2D( lightColor, lightColorUV );
				...
				if ( ( dot( nodeVar34, vec4( vec3( 0.2126, 0.7152, 0.0722 ), 1.0 ) ) > 0.001 ) ) {
					...
					if ( ( nodeVar36 > 0.001 ) ) {
						...
						nodeVar42 = texture2D( nodeUniform3, nodeVar39 );
		...
		for ( uint i = 0u; i < STEP_COUNT_1; i ++ ) {
			...
			nodeVar53 = texture2D( nodeUniform0, nodeVar50 ).x;
			sampleViewPosition_1 = ( ( cameraProjectionMatrixInverse * vec4( ( ( vec3( sampleUV_1.x, ( 1.0 - sampleUV_1.y ), nodeVar53 ) * vec3( 2.0 ) ) - vec3( 1.0 ) ), 1.0 ) ).xyz / vec3( ( cameraProjectionMatrixInverse * vec4( ( ( vec3( sampleUV_1.x, ( 1.0 - sampleUV_1.y ), nodeVar53 ) * vec3( 2.0 ) ) - vec3( 1.0 ) ), 1.0 ) ).w ) );
			...
			if ( ( float( nodeVar63 ) > 0.0 ) ) {
				...
				nodeVar68 = texture2D( lightColor, nodeVar65 );
				...
				if ( ( dot( nodeVar68, vec4( vec3( 0.2126, 0.7152, 0.0722 ), 1.0 ) ) > 0.001 ) ) {
					...
					if ( ( nodeVar70 > 0.001 ) ) {
						...
						nodeVar76 = texture2D( nodeUniform3, nodeVar73 );
